...
handlers.accountCreate = (data, callback) => {
  ...
  if(data.method === 'get'){
    ...
    _helpers.getTemplates('accountCreate', templateData, (err,str) => {
      if(!err && str){
...
handlers.sessionCreate = (data,callback) => {
  ...
  if(data.method === 'get'){
    ...
    // Read in a template as a string
    _helpers.getTemplates('sessionCreate',templateData,(err,str) => {
      if(!err && str){
        // Add the universal header and footer
        _helpers.addUniversalTemplates(str,templateData,(err,str) => {
          if(!err && str){
...
handlers.itemsList = (data,callback) => {
  ...
  if(data.method == 'get'){
    ...
    // Read in a template as a string
    _helpers.getTemplates('itemsList',templateData, (err,str) => {
      if(!err && str){
        // Add the universal header and footer
        _helpers.addUniversalTemplates(str,templateData,(err,str) => {
          if(!err && str){
...
handlers.sessionDeleted = (data,callback) => {
  ...
  if(data.method == 'get'){
    ...
    // Read in a template as a string
    _helpers.getTemplates('sessionDeleted',templateData, (err,str) => {
      if(!err && str){
        // Add the universal header and footer
        _helpers.addUniversalTemplates(str,templateData,(err,str) => {
          if(!err && str){
...
handlers.cartCleared = (data,callback) => {
  ...
  if(data.method == 'get'){
    ...
    const templateData = {
      'title' : 'Cart emptied',
      'description' : 'You have emptied the order cart.',
      'body.class' : 'cartCleared'
    };
    // Read in a template as a string
    _helpers.getTemplates('cartCleared',templateData, (err,str) => {
      if(!err && str){
        // Add the universal header and footer
        _helpers.addUniversalTemplates(str,templateData,(err,str) => {
          if(!err && str){
...
handlers.orderPlaced = (data,callback) => {
  ...
  if(data.method == 'get'){
    ...
    // Read in a template as a string
    _helpers.getTemplates('orderPlaced',templateData, (err,str) => {
      if(!err && str){
        // Add the universal header and footer
        _helpers.addUniversalTemplates(str,templateData,(err,str) => {
          if(!err && str){
...
handlers.accountEdit = (data,callback) => {
  ...
  if(data.method == 'get'){
    ...
    // Read in a template as a string
    _helpers.getTemplates('accountEdit',templateData,(err,str) => {
      if(!err && str){
        // Add the universal header and footer
        _helpers.addUniversalTemplates(str,templateData,(err,str) => {
          if(!err && str){
...
handlers.accountDeleted = (data,callback) => {
 ...
 handlers.favicon = (data, callback) => {
  ...
  if(data.method === 'get'){
    ...
    _helpers.getStaticAssets('favicon.ico', (err, data) => {
      if(!err && data){
        // Callback the data
...
handlers.public = (data, callback) => {
  ...
  if(data.method === 'get') {
    ...
    if(trimmedAssetName.length > 0) {
      ...
      _helpers.getStaticAssets(trimmedAssetName, (err, data) => {
        if(!err && data) {
      ...
      console.log('Error', 'could not read trim');
      callback(404);
    }
  } else {
    callback(405);
...
handlers.index = (data, callback) => {
  let templateData = {
    'title': 'Index',
    'description' : 'Your tasty Elraphty Pizza is at the beckon of your finger tips. Intuitive web to make pizza ordering smooth and .easy',
    'body.class' : 'index'
  }
  _helpers.getTemplates('index', templateData, (err, str) => {
    if(!err && str) {
      _helpers.addUniversalTemplates(str, templateData, (err, fstr) => {
        if(!err && str) {
...
handlers.register = (data, callback) => {
  ...
  _helpers.getTemplates('register', templateData, (err, str) => {
    if(!err && str) {
      _helpers.addUniversalTemplates(str, templateData, (err, fstr) => {
        if(!err && str) {
          callback(200, fstr, 'html');
...
handlers.login = (data, callback) => {
  ...
  _helpers.getTemplates('login', templateData, (err, str) => {
    if(!err && str) {
      _helpers.addUniversalTemplates(str, templateData, (err, fstr) => {
        if(!err && str) {
          callback(200, fstr, 'html');
  ...
  if(token) {
    ...
    _data.read('tokens', token, (err, tokenData) => {
      if(!err && data) {
        ...
        _data.read('users', userEmail, (err, userData) => {
          if(!err && userData) {
            ...
            if(userCarts.length > 0) {
              ...
              callback(403, {'Error' : 'You haven\'t created a cart at all kindly create a new one', items})
            }
          } else {
        ...
        callback(500, {'Error' : 'An error occured while creating your cart', items});
            ...
            _data.read('carts', lastCart, (err, cartData) => {
              ...
              if(!err && cartData) {
                ...
                if (cartData.status === 'active') {
                  ...
                  // initiate payment
                  _helpers.payViaStripe(totalAmount, (err) => {
                    if(!err) {
                  ...
                  _data.update('carts', lastCart, cartData, (err) => {
                    if(!err) {
                      ...
                      _helpers.sendMail(userEmail, emailMsg, (err) => {
                        if(!err) {
