...
helpers.getStaticAssets = (fileName, callback) => {
  ...
  if(fileName) {
    fs.readFile(helpers.publicDir+fileName, (err, data) => {
      if(!err && data) {
        callback(false, data);
      } else {
        // console.log('Error reading file', err);
        callback('Error occured while reading file');
      }
    });
  } else {
...
helpers.addUniversalTemplates = (str, data, callback) => {
  ...
  helpers.getTemplates('_header', data, (err, headerString) => {
    if(!err && headerString) {
      helpers.getTemplates('_footer', data, (err, footerString) => {
        if(!err && footerString) {
          let finalString = headerString+str+footerString;
          callback(false, finalString);
        } else {
          callback('Could not read footer ')
        }
      });
    } else {
      callback('could not read header file')
    }
  });
